...
export class Hud {
  ...
  update(w: World): void {
    ...
    this.hearts.setAttribute('aria-label', `${Math.max(0, w.lives)} life points out of ${max}`)
    this.wave.textContent = `Wave ${Math.min(w.waveIndex + 1, w.level.waves.length)}/${w.level.waves.length}`
